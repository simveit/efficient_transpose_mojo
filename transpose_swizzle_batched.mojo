...
fn transpose_kernel_swizzle_batched[
...
def run_transpose_swizzle_batched(ctx: DeviceContext):
    print("== run_transpose_swizzle_batched")
    ...
    for _ in range(num_warmup):
        ctx.enqueue_function[
            transpose_kernel_swizzle_batched[BLOCK_SIZE, BATCH_SIZE]
        ](
            descriptor,
            descriptor_tr,
            grid_dim=(GMEM_HEIGHT // SMEM_HEIGHT, GMEM_WIDTH // SMEM_WIDTH, 1),
            block_dim=(SMEM_HEIGHT * SMEM_WIDTH // BATCH_SIZE, 1, 1),
        )
        ctx.synchronize()
...
def main():
    with DeviceContext() as ctx:
        run_transpose_swizzle_batched(ctx)
